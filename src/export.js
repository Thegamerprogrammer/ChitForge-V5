...
export async function downloadBrief({ form, sliders, portfolioProfile, chits, poiCount, selectedTargets = [], modelInfo, targetMode }) {
  ...
  chits.forEach((chit, index) => {
    ...
    (chit.evidence || []).forEach((e) => children.push(line('Source', `${safe(e.sourceName)} — ${safe(e.organization)} — ${safe(e.publicationDate)}`), line('Source Quality', e.quality || 'LIMITED'), line('Source Status', e.status || 'MANUAL VERIFICATION'), line('Extraction/Provenance', e.extractionStatus || e.verificationReason || e.status || 'MANUAL VERIFICATION'), new Paragraph({ children: [new TextRun({ text: 'Source URL: ', bold: true, color: 'D4AF37' }), link('Open Source', e.url)] }), new Paragraph({ children: [new TextRun({ text: 'DuckDuckGo Bang URL: ', bold: true, color: 'D4AF37' }), link('Open Bang Search', e.bangUrl)] }), line('DDGS Query', e.ddgsQuery || 'Not supplied'), line('Evidence', e.claimSupported || e.claim)));
