...
const progressStages = ['stage_0', 'stage_1', 'stage_2', 'stage_3', 'stage_4'];
const stageLabels = { stage_0: 'Stage 0 — Context Intelligence', stage_1: 'Stage 1 — Portfolio + Agenda Intelligence', stage_2: 'Stage 2 — Target Discovery', stage_3: 'Stage 3 — Accountability Research', stage_4: 'Stage 4 — POI Generation' };
...
function App() {
  ...
  const pushProgress = (next) => { const total = Math.max(1, Math.trunc(Number.isFinite(next.total) ? next.total : 1)); const done = Math.min(total, Math.max(0, Math.trunc(Number.isFinite(next.completed ?? next.done) ? (next.completed ?? next.done) : 0))); const safe = { ...next, done, completed: done, total }; setStatus(safe); setActivity((items) => [{ time: new Date().toLocaleTimeString(), stage: safe.stage, detail: safe.detail, done, total }, ...items].slice(0, 30)); };
...
function ProgressPanel({ status, poiCount, activity }) {
  ...
  const local = status?.total ? status.done / status.total : 0;
  const pct = Math.round(((currentIndex + local) / progressStages.length) * 100);
  return <div className="progress glass-progress" aria-live="polite"><span className="eyebrow">CHITFORGE</span><h2>FIVE-STAGE RESEARCH ENGINE</h2><strong>{stageLabels[status?.stage] || 'Initializing'}</strong><small>{status?.detail || 'Preparing strict gated research.'}</small><div className="bar"><i style={{ width: `${Math.min(100, Math.max(0, pct))}%` }} /></div><b>{Math.min(100, Math.max(0, pct))}%</b><p>{status?.done ?? 0} / {status?.total || poiCount || 1}</p><div className="stageList">{progressStages.map((stage, index) => <span key={stage} className={index < currentIndex ? 'complete' : index === currentIndex ? 'active' : 'pending'}>{index < currentIndex ? '✓' : index === currentIndex ? '●' : '○'} {stageLabels[stage]}</span>)}</div><div className="activityFeed">{activity.map((item, idx) => <p key={`${item.time}-${idx}`}><time>{item.time}</time> <span>{idx === 0 ? '→' : '✓'}</span> {item.detail || stageLabels[item.stage] || item.stage}</p>)}</div></div>;
}

function StageWidgets({ researchPacket, poiCount }) {
  const { masterContext, portfolioAgenda, targetPool, targetIntelligence, poiPacket } = researchPacket;
  return <div className="stageWidgets">
    {masterContext && <DiscoveryWidget title="CONTEXT DISCOVERED" rows={[['Agenda', masterContext.agendaInterpretation || masterContext.agenda], ['Mandate', masterContext.committeeMandate || masterContext.mandate], ['Background guide findings', masterContext.backgroundGuideSummary], ['Portfolio stance', masterContext.portfolioStance], ['Research notes', masterContext.researchNotes?.text], ['Freeze-date interpretation', masterContext.freezeDateAnalysis], ['Key issues', joinList(masterContext.keyIssues)], ['Research priorities', joinList(masterContext.researchPriorities)]]} />}
    {portfolioAgenda && <DiscoveryWidget title="PORTFOLIO + AGENDA INTELLIGENCE" rows={[['Portfolio stance', portfolioAgenda.portfolioPosition], ['Relevant actors', joinList(portfolioAgenda.relevantActors)], ['Institutions', joinList(portfolioAgenda.institutions)], ['Mechanisms', joinList(portfolioAgenda.mechanisms)], ['Treaties', joinList(portfolioAgenda.treaties)], ['Major disagreements', joinList(portfolioAgenda.disagreements)], ['Queries executed', portfolioAgenda.queriesExecuted], ['Sources discovered', portfolioAgenda.sourcesDiscovered]]} />}
    {targetPool && <div className="discoveryWidget"><h2>TARGETS TO OPPOSE</h2><p className="muted">These are the countries ChitForge believes you can challenge.</p><h3>AUTO-DISCOVERED TARGETS</h3>{(targetPool.autoDiscovered || []).map((t) => <TargetRow key={t.id || t.country} target={t} />)}<h3>USER-SELECTED OPPOSITION</h3>{(targetPool.userSelectedOpposition || []).length ? targetPool.userSelectedOpposition.map((t) => <TargetRow key={t.id || t.country} target={t} />) : <p className="muted">No manual opposition selected.</p>}</div>}
    {targetIntelligence && <div className="discoveryWidget"><h2>TARGET INTELLIGENCE</h2>{(targetIntelligence.incidents || []).slice(0, 8).map((i) => <p key={i.id}><b>{i.target}</b>: {(i.claims || []).slice(0, 1).join('; ')}<br /><small>{i.evidence?.[0]?.epistemicStatus || 'REPORTED'} · {(i.corroboratingSources || i.evidenceIds || []).length} source(s)</small></p>)}</div>}
    {poiPacket && <div className="discoveryWidget"><h2>POIs GENERATED</h2><p><b>POIs:</b> {poiPacket.pois?.length || 0}/{poiCount}</p>{(poiPacket.pois || []).map((poi, index) => <p key={`${poi.target}-${index}`}><b>{poi.target}:</b> {poi.poi}<br /><small>{poi.classification} · Evidence IDs: {(poi.evidenceIds || []).join(', ') || '—'} · Sliders: {JSON.stringify(poi.sliderCompliance || {})}</small>{(poi.evidence || []).map((source) => <><br /><a key={source.url} href={source.url} target="_blank" rel="noreferrer">{source.title || source.url}</a></>)}</p>)}</div>}
  </div>;
}
function joinList(value) { return Array.isArray(value) ? value.map((v) => typeof v === 'string' ? v : (v.country || v.name || v.title || JSON.stringify(v))).join(', ') : (value || '—'); }
function DiscoveryWidget({ title, rows }) { return <div className="discoveryWidget"><h2>{title}</h2>{rows.map(([label, value]) => <p key={label}><b>{label}:</b> {joinList(value)}</p>)}</div>; }
function TargetRow({ target }) { return <p><b>{target.country}</b> <small>{target.selectionType}</small><br />Agenda relevance: {Math.round((target.agendaRelevance || 0) * 100)}% · Challengeability: {Math.round((target.challengeability || 0) * 100)}% · Score: {Math.round((target.targetScore || 0) * 100)}% · Evidence: {(target.evidenceIds || []).length}<br />{target.reason || 'Validated Stage 2 opposition target.'}</p>; }
