...
function StageWidgets({ researchPacket, poiCount }) {
  ...
  return <div className="stageWidgets">
    ...
    {poiPacket && <DiscoveryWidget title="POIs GENERATED" rows={[['POIs', `${poiPacket.pois?.length || 0}/${poiCount}`], ['Target countries', joinList([...(new Set((poiPacket.pois || []).map((p) => p.target)))] )], ['Classifications', joinList([...(new Set((poiPacket.pois || []).map((p) => p.classification)))] )], ['Evidence-backed status', poiPacket.shortfall ? `Shortfall ${poiPacket.shortfall}` : 'Validated evidence IDs resolved'], ['Pressure strategies', 'contradiction, legal/treaty, implementation, commitment-vs-record, accountability traps where supported']]} />}
  </div>;
}
...
function TargetRow({ target }) { return <p><b>{target.country}</b> <small>{target.selectionType}</small><br />Agenda relevance: {Math.round((target.agendaRelevance || 0) * 100)}% · Challengeability: {Math.round((target.challengeability || 0) * 100)}%<br />{target.reason || 'Validated Stage 2 opposition target.'}</p>; }
