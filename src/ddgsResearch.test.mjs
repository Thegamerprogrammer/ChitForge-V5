...
import { bangUrl, buildResearchQueries, classifyExtractionFailure, classifySearchFailure, DDGS_LIMITS, DDGS_SCHEDULING, getResearchSourceBudget, getResearchQueryBudget, planResearchBudget, DDGS_TEXT_BACKEND, DDGS_NEWS_BACKEND, discoverResearch, EXTRACTION_STATUS, SEARCH_STATUS, normalizeDdgsQuery, searchWithBackendFallback, extractSource, shouldSearchNews } from './ddgsResearch.js';
...
assert.equal(classifySearchFailure({ status: 403, body: 'Ratelimit' }), SEARCH_STATUS.RATE_LIMITED);
assert.equal(classifySearchFailure({ status: 202, body: 'rate_limit anomaly blocked' }), SEARCH_STATUS.RATE_LIMITED);
...
assert.equal(DDGS_LIMITS.hardMax, 750);
assert.equal(getResearchSourceBudget(50), 150);
assert.equal(getResearchSourceBudget(100), 300);
assert.equal(getResearchSourceBudget(200), 600);
assert.equal(getResearchSourceBudget(250), 750);
assert.equal(getResearchQueryBudget(100), 25);
assert.equal(getResearchQueryBudget(200), 50);
assert(getResearchSourceBudget(200) > getResearchSourceBudget(100));
assert(getResearchSourceBudget(100) > getResearchSourceBudget(50));
assert(Math.abs(getResearchSourceBudget(200) - (2 * getResearchSourceBudget(100))) <= 1);
assert(getResearchQueryBudget(200) > getResearchQueryBudget(100));
assert(getResearchQueryBudget(100) > getResearchQueryBudget(50));
assert.equal(DDGS_SCHEDULING.searchConcurrency, 1);
assert.equal(DDGS_SCHEDULING.extractConcurrency, 1);
assert.equal(DDGS_SCHEDULING.searchDelay.textMinMs, 4000);
assert.equal(DDGS_SCHEDULING.searchDelay.newsMaxMs, 8000);
assert.equal(DDGS_SCHEDULING.maxExtractedSources, 150);
...
const queryPlan = buildResearchQueries({ form, sliders, selectedTargets: [{ iso: 'AAA', name: 'Target Alpha' }, { iso: 'BBB', name: 'Target Beta' }], targetingMode: 'selected_global', poiTypes: ['LEGAL TRAP', 'AUTO'], poiCount: 100, queryBudget: 60 });
...
assert.equal(scaled.stats.hardMax, getResearchSourceBudget(220));
assert.equal(scaled.stats.targetUrls, getResearchSourceBudget(220));
assert.notEqual(scaled.stats.targetUrls, 80);
assert.equal(scaled.stats.queryBudget, getResearchQueryBudget(220));
assert(scaled.sources.length <= getResearchSourceBudget(220));
assert(scaled.retrievedSources.length <= planResearchBudget({ poiCount: 220 }).extractMax);
assert(planResearchBudget({ poiCount: 220 }).extractMax > 25);
...
const deduped = await discoverResearch({ form: { ...form, freezeDate: '2026-01-01' }, sliders, selectedTargets: [], targetingMode: 'selected_global', poiTypes: [], poiCount: 100, fetchImpl: dedupeFetch, skipHealthCheck: true, delayFn: () => {}, rng: () => 0 });
...
const newsRateCalls = [];
global.fetch = async (url, options) => {
  newsRateCalls.push({ path: new URL(String(url)).pathname, body: JSON.parse(options.body) });
  if (newsRateCalls.length <= 2) return new Response('Ratelimit', { status: 403 });
  return Response.json({ results: [{ date: '2026-01-01T00:00:00+00:00', title: 'Recovered news', body: 'body', url: 'https://news.example/recovered', source: 'Example News' }] });
};
const newsBackoffs = [];
const newsRate = await searchWithBackendFallback('Example Agenda Topic investigation', 5, { endpoint: '/search/news', delayFn: (ms, meta) => newsBackoffs.push({ ms, meta }), rng: () => 0 });
assert.equal(newsRate.results.length, 1);
assert.deepEqual(newsBackoffs.filter((d) => d.meta.kind === 'backoff').map((d) => d.ms), [8000, 16000]);
assert.equal(newsRateCalls.length, 3);

const replayForm = { ...form, agenda: 'Replay Agenda Topic', freezeDate: '' };
let phase = 'initial';
...
const replayFetch = async (url, options) => {
  ...
  if (path === '/extract') return new Response('blocked', { status: 403 });
  const body = JSON.parse(options.body);
  replayRequests.push({ phase, path, query: body.query });
  if (/policy$/.test(body.query)) return new Response('No results found', { status: 500 });
  return Response.json({ results: [sourceResult(`${phase}-${replayRequests.length}`, body.query)] });
};
const initialReplay = await discoverResearch({ form: replayForm, sliders: { aggression: 0, controversy: 0, diplomacy: 0 }, selectedTargets: [], targetingMode: 'selected_global', poiTypes: [], poiCount: 20, fetchImpl: replayFetch, skipHealthCheck: true, delayFn: () => {}, rng: () => 0 });
phase = 'recovery';
const recoveryReplay = await discoverResearch({ form: replayForm, sliders: { aggression: 75, controversy: 70, diplomacy: 20 }, selectedTargets: [], targetingMode: 'selected_global', poiTypes: ['LEGAL TRAP'], poiCount: 100, recoveryFocus: 'missing legal foundation treaty obligation implementation contradiction', researchState: initialReplay.researchState, fetchImpl: replayFetch, skipHealthCheck: true, delayFn: () => {}, rng: () => 0 });
const initialQueries = new Set(replayRequests.filter((request) => request.phase === 'initial' && request.path === '/search/text').map((request) => request.query));
const recoveryQueries = replayRequests.filter((request) => request.phase === 'recovery' && request.path === '/search/text').map((request) => request.query);
assert(recoveryQueries.length > 0);
assert(recoveryQueries.every((query) => !initialQueries.has(query)), 'recovery does not replay initial text queries');
assert(!recoveryQueries.some((query) => /Replay Agenda Topic policy$/.test(query)), 'known-empty query is not replayed in recovery');
assert.equal(recoveryReplay.stats.targetUrls, getResearchSourceBudget(100));
assert(recoveryReplay.sources.length >= initialReplay.sources.length);
assert(recoveryReplay.stats.usedQueryBudget <= recoveryReplay.stats.queryBudget);

let circuitNewsCalls = 0;
const circuitFetch = async (url, options) => {
  const path = new URL(String(url)).pathname;
  if (path === '/extract') return new Response('blocked', { status: 403 });
  const body = JSON.parse(options.body);
  if (path === '/search/news') { circuitNewsCalls += 1; return new Response('Ratelimit', { status: 403 }); }
  return Response.json({ results: [sourceResult(`circuit-${circuitNewsCalls}-${body.query.length}`, body.query)] });
};
const circuit = await discoverResearch({ form, sliders: { aggression: 90, controversy: 90, diplomacy: 0 }, selectedTargets: [{ name: 'Target Alpha' }], targetingMode: 'selected_global', poiTypes: [], poiCount: 80, fetchImpl: circuitFetch, skipHealthCheck: true, delayFn: () => {}, rng: () => 0 });
assert(circuit.stats.newsDisabled, 'news circuit breaker disables repeated throttled news');
assert(circuit.stats.textSearches > circuit.stats.newsSearches, 'text research continues after news circuit breaker');
assert(circuitNewsCalls <= DDGS_SCHEDULING.retryDelays.length + 1, 'news circuit breaker stops further eligible news queries after one bounded throttled search');
