...
export function compactResearchPacket(researchPacket = null, { maxSources = 36, maxExtracted = 18, maxChars = 18000 } = {}) {
  ...
  return { ...compact, sources: sources.slice(0, Math.max(8, Math.floor(sources.length * maxChars / json.length))) };
}
...
export async function recoverShortfall({ form, sliders, selectedTargets, targetingMode, includeFollowUp, mission, poiCount, poiTypes, modelSelection, researchPacket, onProgress }) {
  ...
  const generated = await recoverPoiShortfallLoop({
    ...
    generateCandidates: async ({ level, requestedPoiCount, remainingPoiCount, recoveryRequestCount, current, analysis, recoveryLog }) => {
      ...
      const prompt = buildMissionPrompt({ form, sliders, selectedTargets, targetingMode, includeFollowUp, poiCount: recoveryRequestCount, poiTypes, researchPacket: packet, batchNumber: level, totalBatches: generationSafetyCeiling(requestedPoiCount), previousPoiMetadata: compactPoiMetadata(current.chits) }) + `\n\nRECOVERY INSTRUCTIONS: ${recoveryFocus(level, analysis)} Need exactly ${remainingPoiCount} more final POIs overall. Return ${recoveryRequestCount} candidate POIs in this bounded recovery batch; ChitForge will keep only defensible non-duplicates.` + recoveryContextSummary({ current, requestedPoiCount, remainingPoiCount, recoveryLog });
      const response = await callGemini(form.apiKey, prompt, { ...modelSelection, schema: CHITFORGE_RESPONSE_SCHEMA, attemptKind: 'recovery', onGenerationDiagnostic: (d) => diagnosticProgress(onProgress, { ...d, globalRequestedPoiCount: requestedPoiCount, currentAcceptedCount: current.chits.length, batchRequestedCount: recoveryRequestCount, recoveryRequestedCount: recoveryRequestCount, researchPacketSize: textSize(packet), recoveryContextSize: textSize(compactPoiMetadata(current.chits)) }) });
